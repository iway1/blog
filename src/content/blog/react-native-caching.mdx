...
As such, managing the cache represents a large chunk of the problem that we're trying solve when building common front end apps. In apps with a lot of mutations (frequently updating server data from the front end), we have to do a lot of cache management.

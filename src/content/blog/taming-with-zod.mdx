...
Zod will throw a "ZodError" as soon as you get some bad data, that makes it 100x faster to fix (if you were ever going to fix it) than just having your stuff break in random places.
...
This is why I couldn't use zodios. zodios assumes your function is just an axios call. Mine is not. In vanilla `react-query` is that your query function can be any asynchronous function, including just returning some data from an object like the above.
...
So, in this story we have our backend modeled. Another important design decision to make this all work smoothly is to _couple components to our data types_. In React, this just means typing our props to our zod schema types:
...
Some people avoid coupling at all costs, and believe that no matter what you shouldn't couple components to backend data types. I'm obviously not one of those people, coupling has clear advantages when used correctly and here it was invaluable.
...
HMM. Not even close. What do I do? Should I rebuild the logic for dealing with this end point? That sounds expensive (and it would be).
...
Now everything works. We still validate the data to make sure our code actually knows what the backend is returning at runtime, and then just `.transform` it to what our components are already built to work with.
...
I didn't know this during development, but some times the description is null. This might break stuff because the front end thinks it is always defined. So I simply coerce null into an empty string.
...
The above is a story of using zod in a less-than-ideal situation to get the job done. Ideally, you know what your backend looks like before building out the front end. But here, zod helped me deal with less-than-ideal circumstances and make the project as successful as possible.
